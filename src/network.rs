...
xcore::declare_error!(NetworkError);
...
impl Network {
    ...
    pub fn parse(text: &str) -> Result<Self, NetworkError> {
        ...
        let address: IpAddr = address.parse().map_err(|_| {
            NetworkError::new(format!("'{text}' is not a network as address/prefix"))
        })?;
        ...
        let prefix = match prefix {
            ...
            Some(prefix) => prefix
                ...
                .filter(|p| *p <= widest)
                .ok_or_else(|| {
                    NetworkError::new(format!("'{text}' has a prefix longer than {widest} bits"))
                })?,
        };
